    ...
    uint64_t origsiz, const fido_cred_t *cred)
{
	...
	if ((plaintext.ptr = calloc(1, plaintext.len)) == NULL)
		return -1;
	br_aes_ct64_ctr_init(&ctr, fido_cred_largeblob_key_ptr(cred),
	...
	br_gcm_aad_inject(&ctx, aad, sizeof(aad));
	br_gcm_flip(&ctx);
	memcpy(plaintext.ptr, ciphertext->ptr, plaintext.len);
	br_gcm_run(&ctx, 0, plaintext.ptr, plaintext.len);
	if (!br_gcm_check_tag(&ctx, ciphertext->ptr + ciphertext->len - 16))
		goto out;
