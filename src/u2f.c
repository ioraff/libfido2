...
delay_ms(unsigned int ms, int *ms_remain)
{
	...
	if (sleep_ms(ms * 1000) < 0) {
		fido_log_error(errno, "%s: sleep_ms", __func__);
		return (-1);
	}
...
u2f_get_touch_begin(fido_dev_t *dev, int *ms)
{
	...
	unsigned char	 clientdata_hash[br_sha256_SIZE];
	unsigned char	 rp_id_hash[br_sha256_SIZE];
	unsigned char	 reply[FIDO_MAXMSG];
	int		 r;
	...
	br_sha256_init(&ctx);
	br_sha256_update(&ctx, clientdata, strlen(clientdata));
	br_sha256_out(&ctx, clientdata_hash);
